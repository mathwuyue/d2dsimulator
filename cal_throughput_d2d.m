function throughput = cal_throughput_d2d(SINR_array, n_d2d)
% constans
N_SC = 600;

%% calculate throughput
% RB allocation
RB_allocation_array = ones(N_SC, n_d2d);
...
for i = 1:length(data_rate_array)
    colon1 = colon((i-1)*N_SC*n_d2d+1, i*N_SC*n_d2d);
    throughput_array(idx(:, colon1)) = data_rate_array(i);
end

% throughput
throughput_array = reshape(throughput_array, N_SC, n_d2d)';
throughput = throughput_array * RB_allocation_array;
% actual data rate adjustment
throughput = diag(throughput)'*11/14;
